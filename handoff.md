...
尚未 `git init`，尚未建 GitHub repo／Pages（**對外動作，等使用者確認**）。
`Sync-Agents` 也還沒跑。
...
5. `git init` + 建 GitHub repo（建議名 `weight-maintain-clinic`）+ 開 Pages。
   **建 repo 前先確認 `.gitignore` 有生效、`_local/` 沒被 add 進去。**
...
- **`個案歷程` 尚未驗證**——需要人工在黃色格子 B2／B3 填「測試甲」「1980」才會出資料，
  Drive API 無法代填。驗完記得刪掉測試資料（直接刪 `Form Responses 1` 的兩列，
  歷程與總表是公式會自己更新）
