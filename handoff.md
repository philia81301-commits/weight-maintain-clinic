...
最後一個動作：`個案歷程` 的圖表位置、Y 軸刻度、姓名下拉選單，以及劑量字串去冗字。
**這四項改動需要在 Apps Script 重跑一次 `repair()` 才會生效**（見下方「下一步」）。
...
2. **實際門診用一段時間**，收集不順手之處。後續改進想法列在 `agents.md` 末段，
   但建議用過再決定，實際手感比憑空設想準。
3. **（待決定）是否匯入 xls 的歷史底稿**：xls 每人有 v1／v2 兩個時間點，可產生成
